...
//ForTestData
    ...
    initialize() {
        ...
        // Move test data event listeners into the constructor or a method
    // Add these inside the constructor, after other event listeners:
    if (this.testDataInspectorButton) {
 ...
 addTestDataMessageWithMetadata(content, type, metadata) {
    ...
    getPromptKeys(language, engine) {
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('SELENIUM_JAVA_PAGE_ONLY');
            } else if (this.isTsPlaywright(lang, eng)) {
    ...
    isJavaSelenium(language, engine) {
     ...
     isTsPlaywright(language, engine) {
